---
title: "NuPhy Air75 V3 — 로우프로파일 무선 메카니컬의 새 기준"
date: 2026-03-28
draft: true
tags: ["키보드", "기계식키보드", "NuPhy", "리뷰", "로우프로파일", "무선키보드"]
categories: ["리서치"]
description: "가스켓 마운트, 1200시간 배터리, 스왑 노브까지 — NuPhy Air75 V3 완전 분석"
---

> 출처: [NuPhy 공식](https://nuphy.com/products/nuphy-air75-v3) · [Tom's Guide](https://www.tomsguide.com/computing/peripherals/nuphy-air75-v3-review) · [CraftingWorlds](https://craftingworlds.com/nuphy-air75-v3-review-the-redemption-of-low-profile-keyboards/) · [Tweaktown](https://www.tweaktown.com/reviews/11177/nuphy-air75-v3-low-profile-wireless-keyboard-smooth-operator/index.html)

---

{{< img src="https://nuphy.com/cdn/shop/files/Main01_80323380-e29c-43f8-8edb-e0dc7e4b34d1_1800x1800.png?v=1753784433" alt="NuPhy Air75 V3 메인" width="900" caption="NuPhy Air75 V3" >}}

NuPhy Air75 V3는 로우프로파일 무선 메카니컬 키보드 시장에서 가장 완성도 높은 선택지로 꼽히는 제품이다. V2에서 가스켓 마운트, 압도적 배터리, 스왑 노브까지 실질적으로 모든 것을 업그레이드한 메이저 버전이다.

---

## 디자인 & 외형

{{< img src="https://nuphy.com/cdn/shop/files/Main02_9864a74b-b276-48ad-9300-9b6a8750a2d6_1800x1800.jpg?v=1770183820" alt="NuPhy Air75 V3 측면" width="900" caption="13.2mm 초슬림 프로파일" >}}

- 75% 레이아웃 (84키), ANSI / ISO 지원
- CNC 알루미늄 탑 케이스 + ABS 하단
- 두께 **13.2mm** 초슬림, 무게 약 750g
- 색상: 실버, 블랙
- 3단계 타이핑 각도 조절 (4° / 8° / 10°)
- 노트북 위 사용을 위한 랩탑 그립 고무발 내장

---

## 핵심 업그레이드: 가스켓 마운트

{{< img src="https://nuphy.com/cdn/shop/files/Main03_85272a42-ffdc-4d95-baab-46a8d00efd32_1800x1800.jpg?v=1770183820" alt="NuPhy Air75 V3 타이핑" width="900" caption="가스켓 마운트로 업그레이드된 타이핑감" >}}

V2의 트레이 마운트에서 **가스켓 마운트**로 전환한 것이 이번 업그레이드의 핵심이다. 타이핑 시 진동 흡수가 크게 개선되어 소음이 줄고 타이핑감이 훨씬 부드러워졌다. 스크류인 스태빌라이저도 적용해 스페이스바 등 큰 키의 안정성도 개선됐다.

---

## 스왑 가능한 노브

{{< img src="https://nuphy.com/cdn/shop/files/Main04_7d6f3618-9125-40fb-be8d-7465bc9acbad_1800x1800.png?v=1753784433" alt="NuPhy Air75 V3 노브" width="900" caption="우상단 스왑 노브 — 볼륨/스크롤/매크로 자유 설정" >}}

우상단 키를 일반 키캡 또는 다이얼(노브)로 자유롭게 교체할 수 있다. 볼륨, 스크롤, 밝기 등 원하는 기능을 매핑해 사용 가능하며, 로우/하이프로파일 두 가지 높이의 노브를 선택할 수 있다.

---

## 압도적인 배터리 수명

{{< img src="https://nuphy.com/cdn/shop/files/Main05_f75d9bab-3259-4465-b8ab-993fbc9cdead_1800x1800.jpg?v=1770712215" alt="NuPhy Air75 V3 연결" width="900" caption="3-mode 무선 연결 지원" >}}

| 조건 | 배터리 수명 |
|------|------------|
| RGB 끔 | **최대 1,200시간** |
| RGB 켬 | 60~100시간 |

경쟁 제품 Keychron K3 Max의 약 78시간과 비교하면 차원이 다른 수치다. NuPhy 독자 개발 IO 펌웨어로 전력 최적화를 이룬 결과다.

---

## 연결 & 스위치

{{< img src="https://nuphy.com/cdn/shop/files/2.4_Air75_v3_1800x1800.jpg?v=1770183820" alt="NuPhy Air75 V3 동글" width="900" caption="2.4GHz 동글 수납 슬롯 내장" >}}

**연결 방식 (3-mode)**
- USB-C 유선
- 블루투스 5.1 (최대 3기기)
- 2.4GHz 무선 — 동글 수납 슬롯 내장

**스위치 옵션 (Gateron Nano Switch 3.0)**
- Red Nano — 선형(Linear)
- Brown Nano — 촉각(Tactile)
- Blush Nano — 무소음 선형(Silent Linear)

트래블 거리 3.5mm로 로우프로파일 중 최장축. POM 스템으로 자체 윤활 효과와 내구성을 갖췄다.

---

## 소프트웨어

NuPhy IO 2.0은 설치 없이 브라우저에서 바로 사용하는 웹앱이다. Windows / Mac / Linux 모두 지원하며 키 리매핑, 매크로, 레이어, SOCD 설정을 지원한다.

단, QMK/VIA는 지원하지 않아 오픈소스 기반 깊은 커스터마이징을 원하는 유저에게는 제약이 있다.

---

## 장단점 요약

**👍 장점**
- 로우프로파일 최강 배터리 수명 (RGB 끄면 1,200시간)
- 가스켓 마운트로 대폭 향상된 타이핑감
- 스왑 노브로 편의성 확장
- 2.4GHz 동글 수납 슬롯 내장
- ISO 레이아웃 공식 지원
- 설치 불필요한 웹 기반 소프트웨어

**👎 단점**
- QMK/VIA 미지원
- 노브 품질 아쉬움 (흔들림, 분리 쉬움)
- 일부 블루투스 재연결 지연 이슈
- 색상 선택지 2가지뿐
- 공식 가격 $139 (약 19만 원)

---

## 경쟁 제품 비교

| | Air75 V3 | Keychron K3 Max |
|---|---|---|
| 가격 | $139 | $99~109 |
| 마운트 | 가스켓 | 가스켓 |
| 배터리 | 1,200시간 | 78시간 |
| QMK/VIA | ❌ | ✅ |
| 노브 | ✅ | ❌ |
| ISO 지원 | ✅ | ✅ |

---

## 총평

> "로우프로파일 무선 메카니컬 키보드 시장에서 현재 가장 완성도 높은 선택지."

배터리·타이핑감·휴대성 셋 다 잡고 싶은 유저라면 현 시점 최선의 선택이다. QMK/VIA가 반드시 필요하다면 Keychron K3 Max가 더 적합하지만, 그렇지 않다면 Air75 V3가 한 수 위다.
